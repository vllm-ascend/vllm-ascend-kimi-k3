...
class AscendMLAAttentionSpec(MLAAttentionSpec):
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        return cls(
            ...
            head_size_v=common_fields["head_size_v"].pop(),
            scale_dim=specs[0].scale_dim,
            scale_dtype=specs[0].scale_dtype,
            sparse_head_dim=specs[0].sparse_head_dim,
            dtype=specs[0].dtype,
            kv_quant_mode=common_fields["kv_quant_mode"].pop(),
            page_size_padded=common_fields["page_size_padded"].pop(),
            sliding_window=sliding_window,
            attention_chunk_size=attention_chunk_size,
            cache_dtype_str=cache_dtype_str_set.pop(),
            alignment=common_fields["alignment"].pop(),
            compress_ratio=common_fields["compress_ratio"].pop(),
            model_version=common_fields["model_version"].pop(),
            cache_sparse_sfa_c8=specs[0].cache_sparse_sfa_c8,
            cache_sparse_li_c8=specs[0].cache_sparse_li_c8,
            c8_k_cache_dtype=common_fields["c8_k_cache_dtype"].pop(),
            c8_k_scale_cache_dtype=common_fields["c8_k_scale_cache_dtype"].pop(),
