...
ge::graphStatus Tiling4ChunkKdaBwdIntra(gert::TilingContext *context)
{
    ...
    ChunkKdaBwdIntraTilingContext ctx{
        context->GetNodeName(),
        context->GetInputTensor(KDA_BWD_Q_IDX),
        context->GetInputTensor(KDA_BWD_K_IDX),
        context->GetInputTensor(KDA_BWD_GK_IDX),
        context->GetInputTensor(KDA_BWD_BETA_IDX),
        context->GetInputTensor(KDA_BWD_DAQK_IDX),
        context->GetInputTensor(KDA_BWD_DAKK_IDX),
        context->GetInputTensor(KDA_BWD_DQ_IDX),
        context->GetInputTensor(KDA_BWD_DK_IDX),
        context->GetInputTensor(KDA_BWD_DB_IDX),
        context->GetInputTensor(KDA_BWD_DG_IDX),
        qDesc->GetDataType(),
        *chunkSize,
        *safeGate,
        *layoutMode,
