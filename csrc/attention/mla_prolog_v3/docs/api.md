...
| 通路 | API/入口 | 支持情况 |
| --- | --- | --- |
| Kimi K3 / vllm-ascend 单算子入口 | `torch.ops._C_ascend.mla_prolog` | 支持（Ascend950 编译开启 `ASCEND_PLATFORM_950`） |
...
| `rope_sin` / `rope_cos` | 必选 | 合轴 `(T,Dr)` 或非合轴 `(B,S,Dr)` | BF16 | ND | RoPE 参数 |
...
| `do_rope` | bool | `true` | `{false,true}` | 是否对 Qr/Kr 做 RoPE |
...
aclnnStatus aclnnMlaPrologV3WeightNzGetWorkspaceSize(
    ...
    double qcQrScale, double kcScale, bool doRope,
...
query, query_rope, dequant_scale_q_nope, query_norm, dequant_scale_q_norm = (
    torch.ops._C_ascend.mla_prolog(
        ...
        *,
        ...
        qc_qr_scale=1.0, kc_scale=1.0, do_rope=True,
    )
)
```
...
query, query_rope, *_ = torch.ops._C_ascend.mla_prolog(
    ...
    cache_index=cache_index, cache_mode="PA_BSND", do_rope=True)
...
- Torch / CMake 入口当前按 **Ascend950** 门控（`ASCEND_PLATFORM_950`）。
...
- `do_rope=false` 时跳过 Qr/Kr 旋转，仍写 cache / 出 query。
