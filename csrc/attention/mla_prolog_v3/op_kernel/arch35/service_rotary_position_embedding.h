                                             ...
                                             LocalTensor<float> scale = LocalTensor<float>())
{
    ...
    if constexpr (enableRope) {
        ...
        // Write-through: keep the same staging as the rope path (kFp32 -> kFp32Output -> O).
        // Casting directly from kFp32Local into outputLocal can hit UB aliasing when the caller
        // packs outputLocal and shareTmpUb in one buffer (full-quant Kr path, ENABLE_ROPE=0).
        DataSyncBarrier<MemDsbT::UB>();
        if constexpr (std::is_same<O, C>::value) {
            DataCopy(outputLocal, kFp32Local, cnt);
            PipeBarrier<PIPE_V>();
        } else {
            DataCopy(kFp32OutputLocal, kFp32Local, cnt);
            PipeBarrier<PIPE_V>();
            Cast(outputLocal, kFp32OutputLocal, RoundMode::CAST_RINT, cnt);
            PipeBarrier<PIPE_V>();
        }
