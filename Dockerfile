...
ARG CANN_IMAGE=quay.io/ascend/cann:9.0.1-910b-ubuntu22.04-py3.12
FROM ${CANN_IMAGE}
...
RUN if [ -n "$APTMIRROR" ]; then \
      sed -Ei "s@https?://[a-z0-9.-]+\.ubuntu\.com@${APTMIRROR}@g" /etc/apt/sources.list; \
    fi && \
    apt-get update -y && \
    ...
    if [ "${PIP_INDEX_URL#http://}" != "${PIP_INDEX_URL}" ]; then \
      python3 -m pip config set global.trusted-host "$(printf '%s' "${PIP_INDEX_URL}" | awk -F/ '{print $3}')"; \
    fi && \
